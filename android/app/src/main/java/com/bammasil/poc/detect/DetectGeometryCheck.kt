    ...
    fun run(dstW: Int, dstH: Int): Result {
        ...
        for (degrees in DetectContract.SUPPORTED_ROTATIONS) {
            for (src in SOURCES) {
                ...
                // 🔴🔴 **더 나쁘다: 이 검사의 기대값은 FLIP_Y를 그대로 읽어 만든다**
                //   (아래 expectY0/expectY1). 즉 **오라클이 피검체를 참조하므로 FLIP_Y가
                //   true든 false든 통과한다.** 2026-08-30에 그 값이 실제로 틀린 채(false)
                //   화면이 어긋났는데 이 검사는 passed=true였다 — 그것이 이 구조의 값이다.
                //   ⚠ **고치지 않고 남긴다**: 여기서 기대값을 하드코딩하면 "축 하나가 NDC
                //   전 구간을 덮는가"라는 이 검사의 본래 질문까지 FLIP_Y에 묶인다.
                //   대신 **검사 6**이 mapBox를 회전까지 태우고 기대값을 상수와 무관하게
                //   하드코딩해 그 사각을 메운다. 두 검사를 함께 읽을 것.
