...
enum class RenderArm(
    ...
    companion object {
         ...
         * 측정값이다.**
         *
         * 🟢 [com.bammasil.poc.gl.OverlaySmoother]의 히스토리 비트 폭(`HISTORY_MASK`)이
         * **이 값에서 파생된다** — 여기만 고치면 창이 따라 넓어진다.
         */
        const val OVERLAY_ENTRY_WINDOW_PUBLISHES_MEASUREMENT_VALUE = 3
        ...
        const val OVERLAY_NO_FLICKER_DESIGN =
            "설계에서 막은 것 넷: " +
                ...
                "않았다. 블렌딩 자체를 켜지 않는다. **페이드아웃도 없다** — 잔상을 부드럽게 " +
                ...
                "걸면 '3중 2회'가 '연속 2회'와 같아져 진입 규칙이 사문화된다. " +
                ...
                "hold(18프레임)가 그 끊김을 메우고 있었다. 🔴 **실측 예고(run_ts=20260828_185222 " +
                "= 08-24 런 20260824_212554 재분석)**: 고립 끊김 4.349회/분 · 0인 게시 비율 " +
                "0.4345 · **탐지 양성 버스트 길이 positive_run_publishes p50=2.0게시" +
                "(n=28, min 1)** — 버스트의 절반이 2게시 이하인데 진입이 " +
                "entry_hits_required=2회 지지를 요구하므로, **1게시짜리 버스트는 한 번도 " +
                "그려지지 않고 2게시짜리는 최대 1게시만 보인다.** 이 맞바꿈은 하네스의 " +
                "overlay_ghost.pending과 overlay.flicker가 사후에 판정한다. " +
