...
object SessionWriter {
     ...
     * 붙는다(v4→v5 때와 같은 논거다).
     *
     * v8에서 ④ **면적** 열 하나가 늘었다: `overlay_fill_frac`
     * (`FrameLogRecorder.OVERLAY_HEADER`). 값은 **Σ(박스 면적) ÷ 화면 면적**이고 겹침을
     * 보정하지 않는 float다(0은 정상값 = 박스가 없었다). 🔴 **개수 열 `overlay_boxes`와 짝을
     * 이룬다** — 박스 안쪽을 채우기 시작해서 I칸의 설명 변수가 개수에서 면적으로 바뀌었기
     * 때문이다(`overlay.fill_deviation` (b)). 시간이 아니라 비율이므로 GPU 열이 아니고
     * `gpu_sum_ms`에 들어가지 않는다.
     *
     * 🔎 **이번에는 앱이 먼저 올라갔다** — 위 v5·v7 사례와 방향이 반대이므로 적어 둔다:
     * 이 앱이 실제로 내기 시작한 열이라 버전을 올리지 않으면 CSV에 있는 열을 세션이
     * 부인하게 된다. 하네스(`lib/frame_log.py`)는 **같은 라운드에서 뒤이어 v8로 승급했고**,
     * v7 로그는 양쪽 모두에서 그대로 읽힌다(누락 열을 이름으로 지목하는 경고만 붙는다).
     */
    ...
    private fun buildOverlay(facts: SessionFacts): JSONObject {
        ...
        json.put(
            ...
            "glEnable(GL_BLEND) ; glBlendFuncSeparate(GL_SRC_ALPHA, " +
                ...
                "같다. " +
                "🔴 **glDisable을 같은 함수 안에서 부른다** — 상태가 present 패스까지 남으면 " +
                "다른 arm과 조건이 달라지고 그 차이는 로그 어디에도 남지 않는다. 박스가 " +
                "0개인 프레임은 드로우콜 자체를 내지 않으므로 glEnable도 부르지 않는다. " +
                "⚠ 스트로크의 알파는 1.0이라 검정 밑선·본선 픽셀은 블렌딩 OFF 때와 **비트 " +
                "단위로 같다**(src·1 + dst·0). " +
                "🔴 **정적 상수 알파이며 시간 변조가 없다** — 알파는 정점에 실린 빌드 상수이고 " +
                "프레임 간에도 시간에 따라서도 변하지 않는다(overlay_no_flicker_design (1))"
        )
        json.put(
            "fill_draw_order",
            if (fill) {
        ...
        json.put(
            ...
            "사각형 quad(${HighlightOverlay.GL_PRIMITIVE_NAME}). 박스당 정점 " +
                ...
                "칠하는 것은 **박스 면적의 합**이지 화면 전체가 아니다 — 박스가 0개인 " +
                "프레임에서는 한 픽셀도 칠하지 않으므로 그 판단은 여전히 유효하다"
        )
    ...
    private fun buildPreviewTransform(facts: SessionFacts): JSONObject = JSONObject()
        ...
        .put(
            ...
            "🔴 **영상(present 정점 uPositionMatrix)에 건 회전이며, ④ 박스의 회전이 " +
                ...
                "🏆 **박스도 그 경로에서 맞는다**(2026-08-31) — 박스가 0+90=90을 지도록 " +
                "식을 고쳤고 회전 예산 불변식도 성립한다. ⚠ 카드보드는 **스탬프된 런이 " +
                "아직 0건**이라 근거가 야외 육안뿐이다. " +
    ...
    /**
     * 🔴 **회전 예산 불변식** — ④ 박스는 `texMatrix`가 영상에 건 회전 T를 그대로 진다.
     *
     * ```
     * box_rotation_degrees ≡ camera_rotation_degrees + present_rotation_degrees  (mod 360)
     * ```
     *
     * 왜 합인가: ④ 오버레이는 present **앞의** FBO_A에 그려지므로(`PassthroughRenderer`의
     * 패스8, 패스9가 present) **present는 영상과 박스를 함께 돌리는 공통 모드**이고, 따라서
     * 박스가 메워야 할 몫에서 **빠지지 않는다.** 박스가 메우는 것은 T 하나이며
     * `T = camera + present`다 — 세로 `90+0` · 카드보드 `0+90`, 둘 다 90이다.
     * ⚠ **옛 식 `box + present ≡ camera`는 반증됐다**(2026-08-31): present를 박스 몫에서 빼는
     * 식이라 카드보드에서 박스가 0으로 떨어져 90°가 통째로 빠졌고, 세로는 present가 0이라 두
     * 식의 값이 같아 드러나지 않았다.
     * 🔴 **한계 — 이 검사는 항진명제에 가깝다.** `box`가 같은 두 입력에서 계산되므로
     * `BOX_ROTATION_CLOCKWISE`가 true인 한 `consistent`는 **정의상 참**이다(옛 식도 같았다).
     * **새 식을 반증할 수 있는 기계는 저장소에 없다.**
     * ⚠ **`hasCameraTransform=false` 경로에서는 이 검사가 거짓 경보를 낸다** — 그쪽은 T=0이라
     * 박스가 0인데 식은 `2×camera`를 기대한다. 그 경로는 이 기기에서 한 번도 밟히지 않아
     * **미검증**이고, 검증되지 않은 가정을 코드로 굳히지 않으려고 분기를 넣지 않았다.
     *
     ...
     *
     * 🟢 **카드보드에서도 성립한다** — `targetRotation = ROTATION_90`이라 present가 90(팀원
     * 원본 `e387ae9`의 의미)이고 `rotationDegrees`는 0이므로 박스는 `0 + 90 = 90`이다.
     * 세로의 `90 + 0 = 90`과 **같은 값**이라, 이 식으로 바꿔도 실기기로 확정된 세로 동작은
     * 문자 그대로 안 바뀐다.
     * ⚠ **카드보드 경로는 스탬프된 런이 아직 0건이다** — 영상·박스 모두 야외 육안 확인만
     * 있다(2026-08-31). 카드보드의 기하·튜닝 코드는 팀원 소유이며 우리가 건드리지 않는다.
     *
