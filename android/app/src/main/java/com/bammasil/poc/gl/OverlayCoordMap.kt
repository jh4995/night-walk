 ...
 * ## 🔴 세로 축 방향은 **가정이다**
 *
 * 표시 경로는 방향 보정을 하지 않고(알려진 이슈 13) 패스1의 OES 샘플링은 드라이버가 준
 * `texMatrix`를 그대로 쓴다. 그래서 "센서의 y=0 행이 화면 위인가 아래인가"를 **기계로 확인할
 * 수단이 이 앱에 없다.** [FLIP_Y]는 그 가정 하나를 담은 **유일한 스위치**이며, 화면에서
 * 위아래가 뒤집혀 보이면 **여기만** 바꾼다.
 *
...
object OverlayCoordMap {
    ...
    const val ASSUMPTIONS =
        "🔴 **(1) 세로 축 방향은 가정이다(FLIP_Y=$FLIP_Y).** 표시 경로가 방향 보정을 하지 " +
            "않고(알려진 이슈 13) 패스1의 OES 샘플링은 드라이버가 준 texMatrix를 그대로 " +
            "쓰므로, '센서 y=0 행이 화면 위인가 아래인가'를 기계로 확인할 수단이 앱에 없다. " +
