# `c4f_distill_11n_640` — INT8 양자화 산출물

> 만든 날 **2026-08-31** · 원본 `bammasil_det_c4f_distill_11n_640_640.onnx` (FP32 · 10.61 MB)
...
| 파일 | 무엇 | 크기 |
|---|---|---|
| `bammasil_det_c4f_distill_11n_640_640-INT8_calib_manifest.txt` | 캘리브레이션에 **실제로 쓴** 파일·프레임 번호 159줄 (재현용) | 9.1 KB |
| `bammasil_det_c4f_distill_11n_640_640-INT8_generic.onnx` | ★ **배포 후보 — `generic` 프리셋** · QDQ · per-channel · 가중치 QInt8 / 활성 QUInt8 · 대상 NNAPI · Core ML · ORT CPU EP | 3.15 MB |
| `bammasil_det_c4f_distill_11n_640_640-INT8_metadata.json` | 기계 판독용 — 설정 · SHA256 · 그래프 입출력 · 캘리브 표본 · FP32 대비 비교 결과 | 11.2 KB |
| `bammasil_det_c4f_distill_11n_640_640-INT8_opset13_fp32.onnx` | 중간물 — 원본을 opset 12→13 변환 + `quant_pre_process` 한 것. **FP32 그대로**이고 양자화의 **입력**이다. 배포물이 아니며 지워도 재실행하면 다시 생긴다 | 10.59 MB |
| `bammasil_det_c4f_distill_11n_640_640-INT8_qnn_qdq.onnx` | ★ **배포 후보 — `qnn` 프리셋** · QDQ · per-tensor · 가중치 QUInt8 / 활성 QUInt8 · 대상 스냅드래곤 **QNN EP(HTP)** | 3.04 MB |
| `bammasil_det_c4f_distill_11n_640_640-INT8_README.md` | 이 문서 | — |

### FP32 대비 정합 — 같은 이미지에 두 모델을 먹여 박스를 짝지은 결과

| 파일 | 프리셋 | 가중치 스케일 | 크기 | 검출수 불일치 | 좌표 최대오차(px) | conf 최대오차 |
|---|---|---|---|---|---|---|
| `bammasil_det_c4f_distill_11n_640_640-INT8_generic.onnx` | generic | per-channel | 3.15 MB | 0 | 7.713 | 0.1093 |
| `bammasil_det_c4f_distill_11n_640_640-INT8_qnn_qdq.onnx` | qnn | per-tensor | 3.04 MB | 0 | 20.4583 | 0.08179 |
...
(→ `outputs/export/c4f_distill_11n_640/README.md`). 앱이 고칠 것은 없다.
...
실제로 쓴 파일·프레임 목록은 `bammasil_det_c4f_distill_11n_640_640-INT8_calib_manifest.txt` 에 있다.
...
| 파일 | 1회 추론(ms) |
|---|---|
| `bammasil_det_c4f_distill_11n_640_640.onnx` (FP32) | 39.65 |
| `bammasil_det_c4f_distill_11n_640_640-INT8_generic.onnx` | 45.22 |
| `bammasil_det_c4f_distill_11n_640_640-INT8_qnn_qdq.onnx` | 63.06 |
