...
> 생성 2026-08-03 · 모델 `bammasil_det_c4b_loli0_640.onnx`
...
| 아키텍처 | YOLO11n (ultralytics) |
| 입력 | `images` · `[1, 3, 640, 640]` · `tensor(float)` |
| 파라미터 정밀도 | FP32 |
| opset | 12 |
| 파일 크기 | 10.6 MB |
| SHA-256 | `d038191781202824d660b3a5d9f03a97e8ece5302b35c0fc140b0985198bf5f4` |
...
| 이름 | shape | dtype |
|---|---|---|
| `output0` | `[1, 6, 8400]` | `tensor(float)` |

그래프에 **NMS 가 없다** (의도된 것 — 모바일 NPU 호환).
출력 `output0` 은 `[1, 6, 8400]` 이며 축 구성은
...
NightOwls recording 34 (held-out, 학습 미사용) · person 클래스 기준 (`c4b_loli0`):
...
| recall | 0.609 |
| precision | 0.787 |
| `stairs` 야간 오탐률 (rec 34) | 0.2% |
| `stairs` 야간 오탐률 (rec 38) | 0.0% |
...
```powershell
Get-FileHash bammasil_det_c4b_loli0_640.onnx -Algorithm SHA256
# → d038191781202824d660b3a5d9f03a97e8ece5302b35c0fc140b0985198bf5f4
```

## 재현

```powershell
uv run python scripts/export_onnx.py --imgsz 640 --opset 12
```

원본 가중치: `outputs/detect/c4b_loli0/weights/best.pt` (SHA-256 `b02f4af288a74495…`)
