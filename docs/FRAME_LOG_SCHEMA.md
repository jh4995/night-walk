...
④ 오버레이 열 4개(v7에 셋, v8에 하나)는 성질이 달라 아래 별 절에 있다 → **"④ 오버레이 열"**.
...
### ④ 오버레이 열 (v7 추가, v8 확장) — **GPU 열이 아니다**

③ 탐지 결과를 ④ 오버레이에 이어 붙이면서 들어온 열 3개 + v8에서 늘어난 면적 열 1개다.
프레임당 1행(`frames.csv`)이지만 **위 GPU 열들과 물리량·시계·폐기 가드가 전부 다르다.**

앱이 쓰는 헤더 순서 그대로다(`FrameLogRecorder.OVERLAY_HEADER`):
`stage_h_ms,overlay_boxes,t_overlay_source_ns,overlay_fill_frac` — **면적 열은 묶음의 맨 뒤**다.
...
| `overlay_fill_frac` | float 비율 | **Σ(박스 면적) ÷ 화면 면적** (소수 6자리) | — (비율) | **없음** | `>= 0` | **v8** |

🔴 **이 묶음은 `usesDynamicHighlightBoxes` arm에서만 나간다** — 면적 열도 **같은 게이트**다.
정적 더미 arm(`highlight_boxes` / `_stress` / `_1q`)의 CSV에는 이 열들이 **아예 없다**.
재지 않은 열을 헤더에 넣고 `-1`로 채우면 하네스가 "쟀는데 못 얻었다"로 읽는데 그건 다른 뜻이다.
...
#### `overlay_fill_frac` (v8) — 🔴 **면적 축. 시간도 개수도 아니다**

**무엇인가:** 그 프레임에 그린 박스들의 **면적 합을 화면 면적으로 나눈 값**. NDC에서
`Σ|(x1−x0)(y1−y0)| / 4`이며 앱이 **소수 6자리 고정**으로 쓴다(`0.123456`).

**왜 생겼나:** ④가 박스 **안쪽을 클래스 색으로 채우기** 시작했다. 그전까지 오버레이는 얇은
스트로크 quad뿐이라 칠해지는 픽셀이 테두리 면적뿐이었고, 08-26 S3에서 **I칸이 박스 개수와
거의 무관**하다는 결론까지 나왔다. fill은 **박스 면적만큼** 프래그먼트를 칠하므로 그 결론의
전제가 깨진다 — I칸의 설명 변수가 **개수 하나에서 개수와 면적 둘**로 늘었다.
`overlay_boxes`(스트로크 = 개수·둘레) ↔ `overlay_fill_frac`(fill = 면적)이 **짝**이다.

| 성질 | 규약 |
|---|---|
| 겹침 | 🔴 **보정하지 않는다 — 값이 1을 넘을 수 있다.** 두 박스가 겹치면 겹친 면적이 두 번 더해진다 |
| 뷰포트 밖 | 🔴 **클리핑을 보정하지 않는다.** 화면 밖으로 나간 부분도 그대로 더해진다 |
| 따라서 | 🔴 **"칠해진 픽셀의 비율"이 아니다. 면적의 *합*이다.** 비율이라고 상한 1을 가정하면 I칸 분석이 틀린다 |
| `0.0` | **정상값이다** — 그 프레임에 그린 박스가 없었다(야간 보행에서 다수 프레임이 그렇다) |
| `-1` | **기록하지 않았다.** `overlay_boxes`와 글자 그대로 같은 규약이며 폐기 하한도 같다(`>= 0`) |
| 버짓 칸 | **없다.** 비용이 아니라 **비용의 조건**이다(`overlay_boxes`와 같은 취급) |

🔴 **어떤 ms 합에도 넣지 않는다** — `gpu_sum_ms`·`stage_d_total_ms`는 시간의 합이고 이건
무차원 비율이다. 🔴 **`FRAME_COUNT_COLUMNS`에도 넣지 않는다** — 폐기 하한(`>= 0`)은 같지만
그 범주는 `_to_int`로 읽히므로 `0.123456`이 **조용히 `0`으로 잘린다.** 그래서 `lib/frame_log.py`에
범주가 하나 더 있다: `FRAME_RATIO_COLUMNS`(**가드는 카운트와 같고 파싱은 시간 열과 같다**).
두 사고 모두 상수 자기검사가 **import 시점에** 막는다.

- 🔴 **앱에 요구하는 자릿수: 소수 6자리.** 줄이면 작은 박스가 `0.000`이 되고, 하네스는 그
  샘플을 "면적 0"으로 읽는다(폐기되지 않으므로 **보이지도 않는다**). `stage_h_ms`의 소수 3자리
  요구와 같은 논거인데, 이쪽은 0이 폐기가 아니라 **정상값**이라 더 조용하다.
- 하네스는 `overlay_boxes`가 있는데 이 열이 없으면 **경고한다.** 문장이 두 경우를 함께 말한다:
  fill 이전 빌드(선언 `schema_version <= 7`)면 정상이고, fill 빌드인데 열이 없으면 같은 개수의
  큰 박스와 작은 박스를 구분할 수 없다.
- ⚠️ **`session.json`의 `overlay.fill_alpha`와 다른 값이다.** 그쪽은 arm이 **선언한** 알파
...
| `stage_h_ms` · `overlay_boxes` · `overlay_fill_frac`(v8) · `overlay_freshness_ms` | 분포(p50/p95/p99/min/max/count) |
| `budget_cell` | `stage_h_ms → H`, 나머지는 **명시적 `null`**(면적 비율도 칸이 없다) |
| `condition_note` | 🔴 H칸·I칸은 **박스 개수의 함수**이므로 개수 분포를 함께 옮겨야 한다. **v8부터 조건이 둘**이다 — fill이 들어오면서 **면적**이 더해졌다 |
| `fill_frac_note` (v8) | 🔴 `overlay_fill_frac`은 **"칠해진 픽셀 비율"이 아니다**(겹침 미보정 → 1 초과 가능) · `0.0`은 정상값 · 버짓 칸 없음 |
| `flicker` | 아래 깜빡임 검출기 |
...
| `fill_deviation` (v8) | 🔴 **상류 '비채움' 명세 이탈의 기록.** `upstream_deviation` **옆자리**에 둔다(`no_red_reason` ↔ `person_color_deviation`과 같은 논거) — `fill` 문장만 읽고 "내부는 안 건드린다"로 오독하는 것을 막는다 |
...
패스의 비용을 따라가므로 패스 구성마다 다르다(④ 오버레이 arm은 +2%, 9패스 arm은 +43%).
하한은 **그 arm에서 직접 재는 수밖에 없고**, 그러려면 arm id가 있어야 한다.
...
| 값 | 무엇을 하는가 |
|---|---|
| `detect_parity_cpu` | ORT CPU EP로 추론하고 **E·F·G 각 경계의 텐서를 파일로 덤프**한다 |
| `detect_parity_nnapi` | 위와 같되 NNAPI EP |
| `detect_parity_xnnpack` | 위와 같되 XNNPACK EP |

🔴 **`_prof` arm과 같은 취급 — 이 arm의 시간은 인용하지 않는다.** 샘플당 ~7MB를 디스크에
쓰므로 그 I/O가 같은 스레드에 있으면 E·F·G가 부풀고, 다른 스레드로 빼도 SoC 자원 경쟁이
남는다. **승격본(`docs/baselines/`)으로 올리지 않는다.** 재는 자리는 이미 있다
(`detect_cpu` 등) — **이 arm은 값을 대조하는 자리이지 재는 자리가 아니다.**

덤프 포맷 규약은 `docs/plans/20260806_detect_parity_dump_format.md`,
생산자는 앱(`DetectParityDumper.kt`), 소비자는 `scripts/detect_parity.py`다.
EP별로 셋을 두는 이유는 `detect_*`와 같고(EP 차이는 arm id로 가른다) 하나 더 있다 —
🔴 **NNAPI가 GPU로 내려가는 것을 이미 실측했고 GPU 경로는 fp16으로 떨어질 수 있다.**
같은 모델·같은 입력에 답이 달라지는 것은 성능 문제가 아니라 **안전 문제**라, PC 대조와
별개로 **폰 EP끼리도 서로 대조한다**.
...
| 선언 > 하네스 | **앱이 하네스보다 앞서 나갔다.** 앱이 새로 넣은 열은 미지 열로 버려지므로 새 지표가 요약에 없다. 하네스를 먼저 올리고 다시 집계할 것. ⚠️ **v8이 실제로 이 방향이었다** — 앱이 `SessionWriter.SCHEMA_VERSION = 8`과 `overlay_fill_frac`을 먼저 냈고 하네스가 뒤따라 올라갔다(§6의 권장 순서와 반대). 그 사이에 찍힌 로그를 옛 하네스로 집계했다면 면적 열이 요약에 없다 — **다시 집계할 것** |
...
>
> ⚠️ **`overlay.fill_alpha`(v8)도 같은 대가를 같은 방식으로 받는다.** v7 이하 승격 베이스라인에는
> 이 키가 없어(`None`) 새 fill 런과 **"조건 다름"으로 뜬다**:
> `session.overlay.fill_alpha: baseline=None vs current=0.3`.
...
| **비율 (frames, v8)** | `FRAME_RATIO_COLUMNS` | 🔴 `>= 0` | 〃 (칸 없음 = `null`) |
| 시각 (파생의 재료) | `FRAME_OVERLAY_SOURCE_COLUMNS` | — | 없음(분포를 내지 않는다) |

> 🔴 **비율을 카운트 목록에 넣지 않는다.** 폐기 하한(`>= 0`)이 같아서 같은 통에 넣고 싶어지는데,
> **읽는 함수가 다르다** — 카운트는 `_to_int`라 `0.123456`이 조용히 `0`으로 잘린다. 폐기로도
> 세어지지 않으므로 그 손실은 어디에도 남지 않는다. 가드가 같다는 것은 범주가 같다는 뜻이 아니다.
...
> | 4 | 필수 파일의 `adb pull` 실패, **0바이트 파일**, **회수할 런이 0개**(`runs/`가 비었거나 런 디렉토리가 하나도 없거나 `--run` 이름이 기기에 없음), **원격 조사 실패**(권한 거부 등), `--run`+`--all` 동시 지정, **`detect.enabled=true`인데 `detect.csv`를 못 가져옴**(반쪽 회수, v6), 그리고 **아래 "이름을 미리 알 수 없는 산출물" 4종** |
...
**출력은 항상 `<out_dir>/<기기 런 이름>/` 아래다.** 평면으로 떨어뜨리지 않는다 — 어느 기기
런에서 온 숫자인지가 경로에 남아야 하고, `--all`이면 런마다 하나씩 생긴다. 그 아래 오는 것:
...
<out_dir>/<기기 런 이름>/
  ...
  session.json               필수
  detect.csv                 선택 (탐지 런만)
  ort_profile_<타임스탬프>.json   패턴 — `*_prof` arm 이 남긴다
  parity/                    디렉토리 — ③ 이식 정확성 대조 덤프
```

#### 🔴 이름을 미리 알 수 없는 산출물 — 고정 목록이 놓치던 것

`frames.csv`·`session.json`·`detect.csv` 셋은 **이름이 고정**이라 목록에 적을 수 있었다.
그런데 앱이 그 밖의 산출물을 내기 시작하면 **회수 스크립트는 그것을 조용히 두고 온다.**
실제로 그랬다 — ORT 프로파일러 arm 이 `ort_profile_<타임스탬프>.json`을 남기는데 이름에
타임스탬프가 들어가 고정 목록에 넣을 수도 없었고, **기기에만 쌓였다**(알려진 이슈 31).

| 대상 | 잡는 법 | 없으면 | 기기에 있는데 못 가져오면 |
|---|---|---|---|
| `ort_profile_*.json` | **패턴**(`OPTIONAL_PULL_PATTERNS`) | 정상 (그 arm 이 아닌 런) | **실패(4)** |
| `parity/` | **디렉토리**(`OPTIONAL_PULL_DIRS`) | 정상 (대조 arm 이 아닌 런) | **실패(4)** |

exit 4 가 되는 새 경우 **4종**: ① 패턴 파일 pull 실패 ② `parity/` pull 실패
③ `parity/`를 가져왔는데 **파일이 0개** ④ `parity/` 안에 **0바이트 파일**.

여기서 "선택"은 ***없을 수 있다***는 뜻이지 ***있는데 두고 와도 된다***가 아니다.
`detect.csv`가 `session.json`의 **선언**을 읽어 판정하는 것과 논거가 다르다 — 저쪽은 없는
파일의 필요 여부를 파일만 봐서는 알 수 없지만, 이쪽은 원격 `ls` 목록에 보이므로 **있다는
사실 자체가 근거**다.

> 🔴 **진짜 수정은 목록이 아니라 경고다.** 이름을 하나 더 적는 것은 그 산출물 하나만 고치고
> **다음 산출물에서 같은 일이 난다.** 그래서 회수 후 런 디렉토리에 **남은 것을 세어 보고**,
> 아는 이름·패턴·디렉토리 어디에도 안 걸린 항목이 있으면 **경고**한다
> (`pull_result.json`의 `unclaimed_remote_entries`). `ls` 자체를 못 읽었으면 그 값은 `null`이고
> — **"없다"가 아니라 "말할 수 없다"** 다.
